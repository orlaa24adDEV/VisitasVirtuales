...
app.post('/api/pois', (req, res) => {
    ...
    const poisDelCentro = data.pois.filter(p => p.centerId === centerId);
...
app.put('/api/pois/:id', (req, res) => {
    try {
        // 1. Leer el archivo y obtener el objeto completo
        const data = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
        const poiId = req.params.id;
        const { centerId, name, description } = req.body;

        data.pois = data.pois.map(p => {
            if (p.id === poiId && p.centerId === centerId) {
                return {
                    id: p.id,
                    centerId: centerId,
                    name: name,
        ...
        res.json({ id: poiId, centerId, name, description });
    } catch (error) {
        console.error('Error al actualizar:', error);
        res.status(500).json({ error: 'Error interno al actualizar el POI' });
    }
});
...
app.delete('/api/pois/:id', (req, res) => {
    ...
    data.pois = data.pois.filter(p => !(p.id === poiId && p.centerId === centerId));
