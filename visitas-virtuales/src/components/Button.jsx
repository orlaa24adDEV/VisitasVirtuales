...
const Button = ({
    ...
    const iconIdx = childrenArray.findIndex(child => 
        ...
        return childrenArray.map((child, idx) => {
            if (idx === iconIdx && isValidElement(child)) {
                ...
                const marginClass = isStart 
                    ? (size === 'small' ? 'mr-1.5' : 'mr-2') 
                    : (size === 'small' ? 'ml-1.5' : 'ml-2');
                
                return (
                    <span key={idx} className={`inline-flex items-center shrink-0 ${marginClass}`}>
                        {child}
                    </span>
                );
            }
